...
function parseSettings(raw: Record<string, any>): Settings {
  ...
  return {
    ...
    qq: {
      appId: typeof raw.qq?.appId === "string" ? raw.qq.appId.trim() : "",
      clientSecret: typeof raw.qq?.clientSecret === "string" ? raw.qq.clientSecret.trim() : "",
      allowedUserIds: Array.isArray(raw.qq?.allowedUserIds)
        ? raw.qq.allowedUserIds.map(String)
        : [],
      groupOpenIds: Array.isArray(raw.qq?.groupOpenIds)
        ? raw.qq.groupOpenIds.map(String)
        : [],
    },
    security: {
      level,
