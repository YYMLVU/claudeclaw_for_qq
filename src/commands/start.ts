...
export async function start(args: string[] = []) {
  ...
  // --- QQ ---
  let qqStopBot: (() => void) | null = null;
  let qqEnabled = false;

  async function initQQ(config: { appId: string; clientSecret: string }) {
    if (config.appId && config.clientSecret && !qqEnabled) {
      const { startQQBot, stopQQBot } = await import("./qq");
      await startQQBot(debugFlag);
      qqStopBot = stopQQBot;
      qqEnabled = true;
      console.log(`[${ts()}] QQ: enabled`);
    } else if ((!config.appId || !config.clientSecret) && qqEnabled) {
      if (qqStopBot) qqStopBot();
      qqStopBot = null;
      qqEnabled = false;
      console.log(`[${ts()}] QQ: disabled`);
    }
  }

  await initQQ(currentSettings.qq);
  if (!qqEnabled) console.log("  QQ: not configured");
  ...
  function updateState() {
    ...
    const state: StateData = {
      ...
      qq: !!(currentSettings.qq.appId && currentSettings.qq.clientSecret),
      startedAt: daemonStartedAt,
      web: {
        enabled: !!web,
