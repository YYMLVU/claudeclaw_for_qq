...
function buildSecurityArgs(security: SecurityConfig): string[] {
  ...
  // When running as root, use --allowedTools with a broad allowlist instead.
  if (!isRoot) {
  ...
  switch (security.level) {
    ...
    case "strict":
      if (isRoot) {
        // Under root, explicitly allow tools minus the restricted ones
        args.push("--allowedTools", "Read,Write,Edit,Glob,Grep,NotebookEdit,Bash,TaskCreate,TaskGet,TaskList,TaskUpdate,WebFetch,mcp__");
      } else {
    ...
    case "moderate":
      ...
      if (isRoot) {
        args.push("--allowedTools", "Read,Write,Edit,Glob,Grep,NotebookEdit,Bash,TaskCreate,TaskGet,TaskList,TaskUpdate,WebSearch,WebFetch,mcp__");
      }
      break;
    case "unrestricted":
  ...
  if (security.allowedTools.length > 0) {
    if (isRoot) {
      // Merge with existing allowedTools
      const existing = args.find((a, i) => a === "--allowedTools" && args[i + 1]);
      if (existing) {
        const idx = args.indexOf(existing);
        args[idx + 1] = args[idx + 1] + "," + security.allowedTools.join(",");
      } else {
