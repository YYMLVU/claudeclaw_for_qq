...
**任务落地位置：**
- job 文件：`.claude/claudeclaw/jobs/*.md`
- 执行日志：`.claude/claudeclaw/logs/`
- cron 标记：`# claudeclaw:launch:<job-name>`
...
**行为说明：**
- 定时任务使用固定项目目录作为 `cwd`
- 执行结果会格式化后回发到创建时的 QQ 会话
- `recurring: false` 的 one-shot 任务执行后会自动移除 cron 条目
- 私聊任务保存并回发使用 `user_openid`
- launch 模式目前依赖 Linux `crontab`
...
2. Bot 将收到的文件下载到 `~/tmp/`
3. Claude 从 `~/tmp/` 或 `~/` 下其他路径读取文件并处理
4. Claude 将结果文件写入 `~/tmp/` 或其他 `~/` 路径
5. Bot 扫描工作区并发送本轮新生成的文件
6. 文件会保留，供后续对话继续使用，直到被明确删除
